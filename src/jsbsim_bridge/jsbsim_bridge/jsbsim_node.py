...
class JSBSimBridge(Node):
    ...
    def __init__(self):
        ...
        self._pose_cov_pub = self.create_publisher(
            PoseStamped, f'{topic_prefix}/pose_ground_truth', 10
        )
        ...
        self.create_subscription(
            Float64MultiArray, f'{topic_prefix}/controls', self._control_callback, 10
        )
        ...
        self.get_logger().info(
            f'[jsbsim:{instance_id}] Loaded {aircraft} @ {update_rate}Hz, topics under /{ns}/'
        )
    ...
    def _read_state(self, fdm):
        ...
        return {
            ...
            'vwx': 0.0,
            'vwy': 0.0,
            'vwz': 0.0,  # angular rates not available in c172p
        }
    ...
    def _publish_state(self, state: dict):
        ...
        status = String()
        status.data = (
            f'ARMED jsbsim/c172p pos=({state["lat"]:.4f},{state["lon"]:.4f},{state["alt"]:.1f})'
        )
