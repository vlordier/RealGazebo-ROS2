...
class TestMoveDistances(unittest.TestCase):
    ...
    def test_system_9_and_10_east(self):
        ...
        for sid in systems_9_10:
            ...
            self.assertEqual(offsets, (0.0, -100.0),
                             f"system {sid} should move -100m east, got {offsets}")

    def test_system_4_7_distance(self):
        for sid in range(1, 9):
            if sid not in (9, 10):
                offsets = self._get_offsets(sid)
                self.assertEqual(offsets, (-300.0, 0.0),
                                 f"system {sid} should move -300m north")
        ...
        for cmd_key, cmd_val in KEY_BINDINGS.items():
            with self.subTest(cmd=cmd_val):
                self.assertIn(cmd_val, px4_supported,
                              f"'{cmd_val}' from key '{cmd_key}' not in px4_ros2")
